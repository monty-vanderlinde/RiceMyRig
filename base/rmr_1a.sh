...
[ "$dev1" ] && [ "$root_encr" ] && [ "$home_encr" ] &&
	[ "$root_size" ] && [ "$mac_spoof" ] && [ "$public_ip" ] && [ "$static_ip" ] &&
	[ "$gateway_ip" ] && [ "$hostname" ] && [ "$username" ] ||
	error_exit "One of the variables have not been set in rmr_1a.sh"
...
printf "\nPartitioning %s with /efi, /, and /home partitions\n" "$dev1"
...
cryptsetup open --type plain -v --cipher serpent-xts-plain64 --key-size 512 --key-file /dev/urandom "${dev1}3" to_wipe_3
block_info "dm-0"
printf "\nWiping %s\n" "${dev1}1"
...
block_info "dm-2"
printf "\nWiping %s\n" "${dev1}3"
dd if=/dev/zero of=/dev/mapper/to_wipe_3 bs="$phys_block_size" count="$phys_blocks" status=progress || error_exit "Failed to wipe data on ${dev1}3"
...
do
	...
	cryptsetup -v --type luks1 --cipher serpent-xts-plain64 --key-size 512 --hash sha512 --verify-passphrase --iter-time 5000 luksFormat "${dev1}3" &&
		break
	printf "Something seems to have gone wrong\n"
	printf "Would you like to try entering the password again (y/n)? > "
	read -r answer
	case "$answer" in
		y*|Y*) ;; # Loop again
		*) error_exit "Failed to create luks container on ${dev1}3" ;;
	esac
done
...
cryptsetup open --type luks "${dev1}3" "${home_encr##*/}" || error_exit "Failed to open $home_encr"
...
pacstrap /mnt base base-devel linux linux-firmware neovim man-db man-pages git shellcheck cryptsetup reflector intel-ucode dash parted pacman-contrib grub efibootmgr ||
...
printf "home\tUUID=%s\tnone\tluks,timeout=180\n" "$(lsblk -lp -o NAME,UUID | grep "$(readlink -f "${dev1}3")" | awk '{print $2}')" >> /mnt/etc/crypttab ||
...
sed -e 's/^HOOKS=.*/HOOKS=\(base udev autodetect keyboard modconf block encrypt filesystems resume fsck\)/g' /mnt/etc/mkinitcpio.conf.bak > /mnt/etc/mkinitcpio.conf ||
...
final_check "lsblk -o NAME,FSTYPE,LABEL,FSAVAIL,FSUSE%,MOUNTPOINT $dev1" "Does the final disk layout look alright?" ||
