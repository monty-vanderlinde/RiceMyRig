...
# Future todo:
# Create variables for swap size, dns server, etc
...
dev2=""                         # i.e. /dev/sdb
dev3=""                         # i.e. /dev/sdc
root_raid=""                    # i.e. /dev/md/root or /dev/md0
root_encr=""                    # i.e. /dev/mapper/root or /dev/dm-0
home_raid=""                    # i.e. /dev/md/home or /dev/md1
...
[ "$dev1" ] && [ "$dev2" ] && [ "$dev3" ] && [ "$root_raid" ] && [ "$root_encr" ] && [ "$home_raid" ] && [ "$home_encr" ] &&
	[ "$root_size" ] && [ "$mac_spoof" ] && [ "$public_ip" ] && [ "$static_ip" ] &&
	[ "$gateway_ip" ] && [ "$hostname" ] && [ "$username" ] ||
	error_exit "One of the variables have not been set in rmr_3a.sh"
...
printf "\nPartitioning %s with /efi, /, and /home partitions, with the latter two in RAID 1 (part 1)\n" "$dev1"
...
printf "\nPartitioning %s with /efi, /, and /home partitions, with the latter two in RAID 1 (part 2)\n" "$dev2"
...
printf "\nPartitioning %s with /efi, /, and /home partitions, with the latter two in RAID 1 (part 3)\n" "$dev3"
...
printf "\nSetting up /root in RAID 1 on %s, %s, and %s\n" "${dev1}2" "${dev2}2" "${dev3}2"
mdadm --create --verbose --homehost="$hostname" --name="${root_raid##*/}" --level=1 --metadata=1.2 --raid-devices=3 "$root_raid" "${dev1}2" "${dev2}2" "${dev3}2"||
	error_exit "Failed to create mdadm array on ${dev1}2, ${dev2}2, and ${dev3}2"

printf "\nSetting up /home in RAID 1 on %s, %s, and %s\n" "${dev1}3" "${dev2}3" "${dev3}3"
mdadm --create --verbose --homehost="$hostname" --name="${home_raid##*/}" --level=1 --metadata=1.2 --raid-devices=3 "$home_raid" "${dev1}3" "${dev2}3" "${dev3}3"||
	error_exit "Failed to create mdadm array on ${dev1}3, ${dev2}3, and ${dev3}3"
...
cryptsetup open --type plain -v --cipher serpent-xts-plain64 --key-size 512 --key-file /dev/urandom "${dev2}1" to_wipe_2
cryptsetup open --type plain -v --cipher serpent-xts-plain64 --key-size 512 --key-file /dev/urandom "${dev3}1" to_wipe_3
cryptsetup open --type plain -v --cipher serpent-xts-plain64 --key-size 512 --key-file /dev/urandom "$root_raid" to_wipe_4
cryptsetup open --type plain -v --cipher serpent-xts-plain64 --key-size 512 --key-file /dev/urandom "$home_raid" to_wipe_5
block_info "dm-0"
printf "\nWiping %s\n" "${dev1}1"
...
block_info "dm-1"
printf "\nWiping %s\n" "${dev2}1"
dd if=/dev/zero of=/dev/mapper/to_wipe_2 bs="$phys_block_size" count="$phys_blocks" status=progress || error_exit "Failed to wipe data on ${dev2}1"
block_info "dm-2"
printf "\nWiping %s\n" "${dev3}1"
dd if=/dev/zero of=/dev/mapper/to_wipe_3 bs="$phys_block_size" count="$phys_blocks" status=progress || error_exit "Failed to wipe data on ${dev3}1"
block_info "dm-3"
printf "\nWiping %s\n" "$root_raid"
dd if=/dev/zero of=/dev/mapper/to_wipe_4 bs="$phys_block_size" count="$phys_blocks" status=progress || error_exit "Failed to wipe data on $root_raid"
block_info "dm-4"
printf "\nWiping %s\n" "$home_raid"
dd if=/dev/zero of=/dev/mapper/to_wipe_5 bs="$phys_block_size" count="$phys_blocks" status=progress || error_exit "Failed to wipe data on $home_raid"
cryptsetup close to_wipe_1
cryptsetup close to_wipe_2
cryptsetup close to_wipe_3
cryptsetup close to_wipe_4
cryptsetup close to_wipe_5
