...
[ "$dev1" ] && [ "$dev2" ] && [ "$dev3" ] && [ "$esp_raid" ] && [ "$root_raid" ] && [ "$root_encr" ] && [ "$home_raid" ] && [ "$home_encr" ] &&
	[ "$root_size" ] && [ "$mac_spoof" ] && [ "$public_ip" ] && [ "$static_ip" ] &&
	[ "$gateway_ip" ] && [ "$hostname" ] && [ "$username" ] ||
	error_exit "One of the variables have not been set in rmr_3b.sh"
...
printf "\nPartitioning %s with /efi, /, and /home partitions in RAID 1 (part 3)\n" "$dev3"
...
printf "\nSetting up /efi in RAID 1 on %s, %s, and %s\n" "${dev1}1" "${dev2}1" "${dev3}1"
mdadm --create --verbose --homehost="$hostname" --name="${esp_raid##*/}" --level=1 --metadata=1.0 --raid-devices=3 "$esp_raid" "${dev1}1" "${dev2}1" "${dev3}1"||
	error_exit "Failed to create mdadm array on ${dev1}1, ${dev2}1, and ${dev3}1"
