...
[ "$dev1" ] && [ "$dev2" ] && [ "$dev3" ] ||
	error_exit "One of the device variables have not been set in wipe_3.sh"
...
[ "$wipe_full_disk" = "true" ] &&
	printf "%s\n" \
	"This script is currently configured to wipe the entirety of ${dev1}, ${dev2}, and ${dev3}" \
	...
	printf "%s\n" \
	"This script is currently configured to wipe the first ${wipe_mib} MiB of ${dev1}, ${dev2}, and ${dev3}" \
...
[ "$wipe_full_disk" = "true" ] &&
	printf "\nWriting zeros over the entirety of each disk\n" &&
	dd if=/dev/zero of="$dev1" status=progress &&
	dd if=/dev/zero of="$dev2" status=progress &&
	dd if=/dev/zero of="$dev3" status=progress

[ "$wipe_full_disk" = "false" ] &&
	printf "\nWriting 1GiB of zeros to the start of each disk\n" &&
	dd if=/dev/zero of="$dev1" bs=1MiB count="$wipe_mib" status=progress &&
	dd if=/dev/zero of="$dev2" bs=1MiB count="$wipe_mib" status=progress &&
	dd if=/dev/zero of="$dev3" bs=1MiB count="$wipe_mib" status=progress

printf "\nInitializing gpt table on %s\n" "$dev1"
parted -s -a optimal -- "$dev1" mklabel gpt
printf "\nInitializing gpt table on %s\n" "$dev2"
parted -s -a optimal -- "$dev2" mklabel gpt
printf "\nInitializing gpt table on %s\n" "$dev3"
parted -s -a optimal -- "$dev3" mklabel gpt
